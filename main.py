...
# ฟังก์ชันบันทึกข้อมูลการสั่งซื้อไปที่ไฟล์ data.env
def save_order_to_env(username, category, product_name, quantity, total_price, link):
    # ตรวจสอบว่าไฟล์ data.env มีอยู่หรือไม่ ถ้าไม่มีก็สร้างขึ้น
    if not os.path.exists("data.env"):
        try:
            with open("data.env", "w") as file:
                # ไม่เพิ่มข้อมูลใดๆ ในไฟล์เมื่อสร้างใหม่
                pass
            print("ไฟล์ data.env ถูกสร้างขึ้นใหม่ ✅")
        except Exception as e:
            print(f"ไม่สามารถสร้างไฟล์ data.env ได้: {e} ❌")
            return
    ...
    # บันทึกข้อมูลลงในไฟล์ data.env
    try:
        with open("data.env", "a") as file:
            file.write(f"\n# Order at {current_time}\n")
            for key, value in order_data.items():
                file.write(f"{key.upper()}={value}\n")
            file.write("\n")
        print("ข้อมูลการสั่งซื้อได้ถูกบันทึกลงในไฟล์ data.env เรียบร้อยแล้ว ✅")
    except Exception as e:
        print(f"ไม่สามารถบันทึกข้อมูลการสั่งซื้อได้: {e} ❌")
    ...
    try:
        ...
        if response_order.status_code == 200:
            ...
            if 'order' in order_data:
                ...
                save_order_to_env(username, category, product['description'], quantity, total_price, link)
            else:
